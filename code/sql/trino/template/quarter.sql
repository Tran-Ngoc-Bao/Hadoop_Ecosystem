create table if not exists hive.datasets_quarter.marketing_airline_network_{year}_{quarter}
with (format = 'parquet')
as select
Year, Quarter, Month,
...
create table if not exists hive.datasets_quarter.flights_across_days_of_the_week_{year}_{quarter}
with (format = 'parquet')
as select
...
create table if not exists hive.datasets_quarter.flight_origins_{year}_{quarter}
with (format = 'parquet')
as select
Year, Quarter, Month,
...
create table if not exists hive.datasets_quarter.flights_cancellations_by_day_of_the_week_{year}_{quarter}
with (format = 'parquet')
as select
Year, Quarter, Month, DayOfWeek,
...
create table if not exists hive.datasets_quarter.total_cancellations_and_flights_per_carrier_{year}_{quarter}
with (format = 'parquet')
as select
...
(select Year, Quarter, Month, count(1) total_flights, count_if(Cancelled = 1) total_cancellations from hive.processed_data.flight_{year} where Quarter = {quarter} group by Year, Quarter, Month) b
where s.Year = b.Year and s.Quarter = b.Quarter and s.Month = b.Month and s.Quarter = {quarter}
group by s.Year, s.Quarter, s.Month, s.Marketing_Airline_Network, b.total_flights, b.total_cancellations;



create table if not exists hive.datasets_quarter.day_of_month_{year}_{quarter}
with (format = 'parquet')
as select
...
from hive.processed_data.flight_{year} s,
(select Year, Quarter, Month, count(1) all, count_if(Cancelled = 1) cancelled from hive.processed_data.flight_{year} where Quarter = {quarter} group by Year, Quarter, Month) b
where s.Year = b.Year and s.Quarter = b.Quarter and s.Month = b.Month and s.Quarter = {quarter}
group by s.Year, s.Quarter, s.Month, s.DayofMonth, b.all, b.cancelled;



create table if not exists hive.datasets_quarter.mean_delay_by_aircraft_carrier_{year}_{quarter}
with (format = 'parquet')
as select
