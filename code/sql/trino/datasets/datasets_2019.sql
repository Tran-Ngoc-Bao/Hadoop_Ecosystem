...
create table if not exists hive.datasets.day_of_month_2019_new
with (format = 'parquet')
as select
...
from hive.processed_data.flight_2019 s,
(select Year, Quarter, Month, count(1) all, count_if(Cancelled = 1) cancelled from hive.processed_data.flight_2019 group by Year, Quarter, Month) b
where s.Year = b.Year and s.Quarter = b.Quarter and s.Month = b.Month
